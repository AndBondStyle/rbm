...
//        this->create_publisher<truck_msgs::msg::IcpOdometryStat>("/icp/odometry/status", 10);
    ...
    signal_.odometry = this->create_publisher<nav_msgs::msg::Odometry>("/icp/odom_custom", 10);
...
nav_msgs::msg::Odometry IcpResult::toOdometryMsg() const {
    ...
    const float dt = (start - end).seconds();

    const float x = t(0, 2);
    const float y = t(1, 2);

    // RCLCPP_INFO(rclcpp::get_logger("icp_odometry"),
    //         "t.x=%.4f t.y=%.4f dt=%.4f",
    //         t(0, 2), t(1, 2), dt);
